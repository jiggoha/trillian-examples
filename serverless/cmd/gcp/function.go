...
func Integrate(w http.ResponseWriter, r *http.Request) {
	...
	if len(privKey) == 0 {
		http.Error(w, "Please set SERVERLESS_LOG_PUBLIC_KEY environment variable",
			http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to instantiate signer: %q", err),
	...
	client, err := storage.NewClient(ctx, os.Getenv("GCP_PROJECT"))
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to create GCS client: %v", err),
			http.StatusBadRequest)
		return
	}
	...
	if d.Initialise {
		if err := client.Create(ctx, d.Bucket); err != nil {
			http.Error(w, fmt.Sprintf("Failed to create bucket for log: %v", err),
				http.StatusBadRequest)
			return
		}
		...
		if err := signAndWrite(ctx, &cp, cpNote, s, client, d.Origin); err != nil {
			http.Error(w, fmt.Sprintf("Failed to sign: %q", err),
				http.StatusInternalServerError)
		}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to read log checkpoint: %q", err),
			http.StatusInternalServerError)
	}
	...
	v, err := note.NewVerifier(pubKey)
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to instantiate Verifier: %q", err),
			http.StatusInternalServerError)
	}
	cp, _, _, err := fmtlog.ParseCheckpoint(cpRaw, d.Origin, v)
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to open Checkpoint: %q", err),
			http.StatusInternalServerError)
	}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to integrate: %q", err),
			http.StatusInternalServerError)
	}
	if newCp == nil {
